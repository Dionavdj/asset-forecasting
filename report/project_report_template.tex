...
\begin{abstract}
\noindent
This project examines the predictive performance of standard econometric models and machine learning methods for equity returns and volatility within a fully reproducible forecasting pipeline. Using Tesla (TSLA) as a high volatility case study, we benchmark a random walk against AR(1), automatically selected ARIMA models, regularized linear regressions and XGBoost over horizons ranging from 10 trading days to one year. To prevent look ahead bias, model selection is based exclusively on a validation window. The selected model is then refit on the combined train+validation sample and evaluated out of sample using price level errors. The main empirical finding is that return forecasts remain weak: once realistic time series validation is enforced, gains over the random walk are not stable. However, volatility is substantially more predictable. A GJR-GARCH(1,1) specification with Student-$t$ innovations consistently improves on an EWMA baseline and yields credible uncertainty bands for Monte Carlo price simulations. Overall, the pipeline proves more reliable as a risk and uncertainty tool than as a point return forecasting engine.
\end{abstract}
...
\begin{itemize}[itemsep=0.35\baselineskip, topsep=0.2\baselineskip]
